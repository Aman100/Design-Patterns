/*
ElectricalsFactory object should be created only once. -> Make it static
But constructor is going to get called number of times objects get created; to 
avoid that
Declare a temp static property. When defining temp property outside the class
make a function call to a static method. It will run once for every object.

Takeaways:
Why init() method can access Private members?
Ans: Class Designer is going to write init() so that's why class designer can
access private members.
Encapsulation is only required why developer doesn't want outsiders to access
private fields
*/
...
static int init()
{
//This method will be called only once in a life time
...
int ElectricalsFactory::useless=ElectricalsFactory::init();
